...
PROMPT_SYSTEM_RULES = """You are analyzing one function from the codebase.
...
- A function is CROSS-CUTTING only when it IS the small utility called from many places (the helper itself), NOT when it merely calls such a utility or calls a shared logging/metrics helper. Route genuine cross-cutting utilities to a `crosscut-*` stage.
- A function that merely uses a shared logger/metrics helper is NOT cross-cutting — it should be assigned to the stage that owns its real work.
- For functions that BELONG to the main flow (the work itself), assign to the matching stage / sub-stage / side flow, even if they happen to also log or measure incidentally.
- Multiple stages allowed when a function genuinely participates in more than one (rare for cross-cuts, which go to `crosscut-*`).
- Trivial public API-SURFACE functions (tiny accessors like `name()`, `version()`, ≤5 lines with no business logic) that are not part of any data-flow stage: return `function_assignments: []` and set `granularity: "function"`. These are recorded as unmapped api_surface entries. Any function with non-trivial work MUST have at least one stage assignment.
- SUBSYSTEM-INTERNAL helpers (defined inside a subsystem module and not its public entrypoint): assign them to the MAIN-FLOW stage that drives their execution, unless the skeleton defines a dedicated `subsys-*` stage for that subsystem.
...
Cover these aspects, in this order, in plain English:
  ...
  3. OUTPUTS / STATE WRITTEN: What it returns and which instance/global state it mutates (fields it sets, collections it appends to, files it writes, etc.).
...
Region-level `purpose` follows the same structure but is shorter (1–3 sentences, 30–80 words) and focused on what makes this contiguous block a distinct narrative step (e.g., "writes a state field", "calls into a helper", "assembles the outgoing request").
...
# downstream pipeline can be tested without hitting the API. Codebase-specific
# name→crosscut hints can be injected via the HANDBOOK_CROSSCUT_NAMES env var
# (JSON object mapping function name → crosscut stage id); empty by default so
# nothing is hardcoded to any one project.
_CROSSCUT_NAMES: dict[str, str] = {}
try:
    _CROSSCUT_NAMES = json.loads(os.environ.get("HANDBOOK_CROSSCUT_NAMES", "") or "{}")
    if not isinstance(_CROSSCUT_NAMES, dict):
        _CROSSCUT_NAMES = {}
...
def main(argv: list[str] | None = None) -> int:
    ...
    default_source_root = Path(os.environ.get("HANDBOOK_SOURCE_ROOT", "."))
