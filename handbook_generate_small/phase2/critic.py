...
ROLE_PROMPTS: dict[str, str] = {
    "engineer": """You are a SENIOR ENGINEER reviewing a proposed change to how a function is classified or organized in a codebase handbook. Your job is to be skeptical and find real concerns rooted in code behavior.
...
Focus on:
  ...
  - Does the proposal misuse cross-cutting categories (e.g. labeling a function as a `crosscut-*` logging helper just because it calls a shared logger)?
...
You may APPROVE, REVISE (with concrete concerns), or REJECT.""",
    ...
    "architect": """You are a SYSTEM ARCHITECT reviewing a proposed change to the structural mapping of a codebase. Your job is to find structural problems.

Focus on:
  - Do stage boundaries stay clean?
  - Does this change cause one stage to bloat (>20 members) or starve (<2 members)?
  - Are subsystem boundaries respected (a `subsys-*` module's internals should not invade a main-flow stage's narrative)?
