...
def _load_json_env(name: str) -> dict[str, str]:
    """Load a JSON object mapping from an env var; empty dict on absence/error."""
    ...
    try:
        ...
        logger.warning("ignoring malformed JSON in %s", name)
        return {}
    return data if isinstance(data, dict) else {}


# Codebase-specific subsystem enrichment (optional, empty by default so nothing
# is hardcoded to any one project):
#   HANDBOOK_SUBSYS_FILE_MAP      {"<source filename>": "<subsys stage id>"}
#       maps an internal file to a subsystem stage; a stage that calls into that
#       file (from a different file) gets the subsys id in its subsystem_refs.
#   HANDBOOK_SUBSYS_BOUNDARY_MAP  {"<boundary-id substring>": "<subsys stage id>"}
#       maps a substring of a boundary callee id (external module path) to a
#       subsystem stage; a stage calling that boundary gets the subsys id.
SUBSYS_FILE_TO_ID = _load_json_env("HANDBOOK_SUBSYS_FILE_MAP")
SUBSYS_BOUNDARY_TO_ID = _load_json_env("HANDBOOK_SUBSYS_BOUNDARY_MAP")
        ...
        for m in info.get("members", []):
            ...
            for edge in edges_by_caller_q.get(qn, []):
                ...
                if callee_id.startswith("boundary:"):
                    for substr, subsys_id in SUBSYS_BOUNDARY_TO_ID.items():
                        if substr and substr in callee_id:
                            refs.add(subsys_id)
                else:
