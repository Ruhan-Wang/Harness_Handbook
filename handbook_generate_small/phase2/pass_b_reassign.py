...
ACTOR_RULES = """You are auditing ONE stage of a codebase handbook for misplaced members.
...
- The member is a small cross-cutting utility (measuring/counting, formatting/
  capping, recording markers, generic logging helpers) that belongs in a
  `crosscut-*` stage but ended up in a main-flow stage.
- The member is subsystem-internal and there is a dedicated `subsys-*` stage
  that fits it better.
