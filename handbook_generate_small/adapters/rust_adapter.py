...
class RustAdapter(LanguageAdapter):
    ...
    def analyze(self, files: list[Path], source_root: Path) -> ModuleAnalysis:
        ...
        for m in modules.values():
            for node, body in m.functions:
                ...
                if body is not None:
                    edges.extend(self._resolve_calls(node, body, m, modules, type_to_module))
    ...
    def _resolve_calls(self, caller: FunctionNode, body: TSNode, mod: _RustModule,
                       modules, type_to_module) -> list[CallEdge]:
        edges: list[CallEdge] = []
        for node, is_await in _iter_calls(body):
            ...
            if fn_expr is None:
                continue
            edge = self._resolve_one(fn_expr, is_await, caller, mod, type_to_module, node.start_row + 1)
            if edge is not None:
                edges.append(edge)
        return edges

    def _resolve_one(self, fn_expr: TSNode, is_await: bool, caller: FunctionNode,
                     mod: _RustModule, type_to_module, line: int):
        raw = fn_expr.text[:80]
        k = fn_expr.kind
