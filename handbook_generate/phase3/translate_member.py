...
TIER3_PROMPT_VERSION = "v2-registers"
...
```json
{
  ...
  "interface": {
    ...
    "reads_state": ["<本函数读取的 self._ 属性，如 self._tmux_pane_width>"],
    "returns": "<返回什么；若为 None，说明真正的产出（副作用）>",
    "side_effects": ["<写入的 self._ 属性 / 外部副作用，如 启动 tmux session>"]
  },
  "execution_flow": [
...
```json
{
  ...
  "interface": {
    ...
    "reads_state": ["<self._ attributes this function reads, e.g. self._tmux_pane_width>"],
    "returns": "<what it returns; if None, state the real product (side effect)>",
    "side_effects": ["<self._ attributes written / external effects, e.g. starts the tmux session>"]
  },
  "execution_flow": [
...
Style reminders for the prose fields:
  - Active voice. "It resets the trajectory" not "the trajectory is reset by it".
  - Concrete verbs. "Build the prompt" not "perform construction of the prompt".
  - Inline backticks for variable names and qualnames: `_trajectory_steps`, `Chat`, `_init_llm`.
...
_PROMPT_HEADERS = {
    "zh": {
        "intro": "你是 Terminus 2 Harness 翻译师。把一个函数（或函数内多个 region）的源码翻译成中文 handbook 条目。",
        "input": "# 输入",
        "unit": "## 翻译单元",
        "stage": "## 所属 stage",
        ...
        "self_check": (
            ...
            "6. 源码里如果有 `self._pending_*` / `self._trajectory_steps.append` / `self._n_episodes += 1` 等模式，是否在 register_interactions 里如实列出？\n\n"
    ...
    "en": {
        "intro": "You are the Terminus 2 Harness translator. Convert one function (or one function's regions) into a handbook entry. Output is JSON only.",
        "input": "# Input",
        ...
        "self_check": (
            ...
            "6. If the source mutates `self._pending_*`, appends to `self._trajectory_steps`, increments `self._n_episodes`, etc. — are those reflected in register_interactions?\n\n"
