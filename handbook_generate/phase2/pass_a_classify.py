...
ACTOR_RULES = """You are analyzing one Python function from the Terminus 2 agent harness, and classifying it into one or more stages of a hand-authored data-flow skeleton.
...
- CROSS-CUTTING utility functions (small utilities called from many places — token counting, output length capping, recording markers) go to crosscut-X* ONLY. Do NOT add them to consuming stages.
- A function that uses self.logger.X is NOT cross-cutting just because of that — assign by its primary identity.
- For api-surface methods (e.g. name(), version()): set function_assignments=[] and granularity="function" (will be recorded as unmapped api_surface).
- For subsystem-internal functions (in tmux_session.py / terminus_*_parser.py / asciinema_handler.py): assign to the main-flow stage that drives them.
...
Look at the caller/callee context provided — your classification should be consistent with their stage assignments. A function that's only called from stage-4.2's region typically belongs near stage-4.2.
