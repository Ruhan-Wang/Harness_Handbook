...
ACTOR_RULES = """You are the SKELETON DOCTOR for a Agent System Harness Handbook. Look at the current `skeleton.yaml` and the current `mapping.yaml` distribution. Propose **at most 3** structural changes to the skeleton that would improve clarity.

WHAT TO LOOK FOR
1. STAGE OVERLOAD: a stage with >20 members; especially if most come from one subsystem file.
...
3. MISSING SUBSYSTEM STAGE: subsystem files (tmux_session.py, parser files, asciinema_handler.py) whose internal helpers are scattered into consuming main-flow stages.
   → Propose `add_stage` for subsystem internals.
...

{
  "action": "add_stage",
  "new_stage": {
    "id": "<unique stage id, e.g. subsys-tmux-internal>",
    "title": "<short title>",
...
def run_pass_c(
    ...
    task_context = (
        f"Skeleton doctor for Terminus 2 harness. "
