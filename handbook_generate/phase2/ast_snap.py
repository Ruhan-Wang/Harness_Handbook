...
def find_function_statements(
    ...
    """Return ``[(lineno, end_lineno), ...]`` for every statement (at any depth)
    in the function's body. None if the function can't be located.

    ``qualname`` is dotted, e.g. ``Terminus2._run_agent_loop`` or just ``foo`` for
    ...
    # Strip the module prefix if present (graph.json qualnames are like ``terminus_2.Terminus2._run_agent_loop``)
    ...
    if len(parts) == 1:
        for node in ast.iter_child_nodes(tree):
            if (
                ...
                return _collect_all_statements(node.body)
        return None
        ...
        if isinstance(node, ast.ClassDef) and node.name == class_name:
            for sub in node.body:
                if (
                    ...
                    return _collect_all_statements(sub.body)
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    default_source = project / "harbor/src/harbor/agents/terminus_2"
