...
def actor_critic_loop(
    ...
    Cost knobs (env, set by Handbook Studio's quality selector):
      - HANDBOOK_MAX_REVISE_ROUNDS: clamp the revise rounds (0 = actor+critic only).
      - HANDBOOK_SKIP_CRITIC: if truthy, accept the actor's proposal with NO critic
        call (1 LLM call per item instead of 2+). Fastest, lowest quality.
    """
    env_rounds = os.environ.get("HANDBOOK_MAX_REVISE_ROUNDS")
    if env_rounds is not None:
        try:
            max_revise_rounds = min(max_revise_rounds, int(env_rounds))
        except ValueError:
            pass
    skip_critic = os.environ.get("HANDBOOK_SKIP_CRITIC", "").lower() in ("1", "true", "yes")
    ...
    # Skip-critic mode: trust the actor, no review call.
    if skip_critic:
        return ActorCriticResult(
            final_proposal=p1,
            rounds=1,
